...
void myproject_float(
    float in_x[N_INPUT_1_1],
    float layer9_out[N_LAYER_8]
) {

    input_t in_x_ap[N_INPUT_1_1];
    nnet::convert_data<float, input_t, N_INPUT_1_1>(in_x, in_x_ap);

    result_t layer9_out_ap[N_LAYER_8];

    myproject(in_x_ap,layer9_out_ap);

    nnet::convert_data<result_t, float, N_LAYER_8>(layer9_out_ap, layer9_out);
}

void myproject_double(
    double in_x[N_INPUT_1_1],
    double layer9_out[N_LAYER_8]
) {

    input_t in_x_ap[N_INPUT_1_1];
    nnet::convert_data<double, input_t, N_INPUT_1_1>(in_x, in_x_ap);
